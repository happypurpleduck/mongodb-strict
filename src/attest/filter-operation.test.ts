...
describe("filterOperations type", () => {
	it("applies operators to object properties", () => {
		attest({} as FilterOperations<{ a: number }>)
			.type
			.toString
			.snap("{ a?: FilterOperators<number> | undefined }");
	});

	it("applies operators to primitive type", () => {
		attest({} as FilterOperations<number>)
			.type
			.toString
	...
	it("handles array type", () => {
		attest({} as FilterOperations<number[]>)
			.type
			.toString
			.snap("(FilterOperators<number> | undefined)[]");
	});
});
